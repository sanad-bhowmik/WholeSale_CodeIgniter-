...
//            echo $this->db->last_query();die();
    ...
    public function userPermission($id = null) {
        $userrole = $this->db->select('sec_userrole.*,sec_role.*')
                        ->from('sec_userrole')
...
//        echo '<pre>';        print_r($acc_tbl);die();
        if ($acc_tbl != NULL) {
            ...
            return $result = $this->db->select("
				...
				menusetup_tbl.menu_title,
				menusetup_tbl.page_url,
				menusetup_tbl.module
				")
                    ->from('role_permission_tbl')
                    ->join('menusetup_tbl', 'menusetup_tbl.id = role_permission_tbl.menu_id', 'full')
