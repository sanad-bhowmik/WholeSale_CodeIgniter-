...
class Payment extends CI_Model {
   ...
   i.*, i.amount as debit,
    i.pay_amount as credit,i.transaction_id,i.relation_id,f.customer_name,,h.supplier_name,k.person_name,m.parent_id');
        ...
        $this->db->join('employee_history e', 'i.relation_id=e.employee_id', 'left');
        $query = $this->db->get();
...
//    ================ its for get_transaction_info =============
    public function get_transaction_info() {
        $sql = 'SELECT si.supplier_name, ci.customer_name, t.*, f.person_name, f.person_id, e.first_name, e.last_name, a.account_name
                    FROM transection t 
                    ...
                    LEFT JOIN person_information f ON f.person_id = t.relation_id
                    LEFT JOIN employee_history e ON e.employee_id = t.relation_id
                    LEFT JOIN account_2 a ON a.account_id = t.relation_id
                    GROUP BY t.transaction_id ORDER BY create_date DESC';
//        echo $sql;die();
...
//    ============ its for employee selected ============
    public function selected_employee($trans) {
        $this->db->select('*');
        $this->db->from('employee_ledger a');
        $this->db->join('employee_history b', 'b.employee_id = a.employee_id');
...
// todays report pop up query
    ...
    // date to date query 
    public function date_summary_query() {
        $today = date('Y-m-d');
        $sql = 'SELECT si.supplier_name, ci.customer_name, t.*, f.person_name, f.person_id, e.first_name, e.last_name
                  FROM transection t 
	LEFT JOIN supplier_ledger sl ON sl.transaction_id = t.transaction_id
                  LEFT JOIN supplier_information si ON si.supplier_id = sl.supplier_id
	LEFT JOIN customer_ledger cl ON cl.transaction_id = t.transaction_id 
...
//
//        //$this->db->where('transection.transaction_id''trans_veiw.transaction_id');
