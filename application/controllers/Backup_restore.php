...
class Backup_restore extends CI_Controller {
    ...
    public function index() {
        $this->permission->check_label('backup')->create()->redirect();
...
//        echo $this->savePath; echo $this->fileName; die();
        $this->load->helper('download');
        $this->load->dbutil();
        $db_format = array('format' => 'sql', 'filename' => 'wholesale.sql');
        $backup = & $this->dbutil->backup($db_format);
        $dbname = "backup" . ".sql";
        $save = 'assets/dbimport/' . $dbname;
...
//    =========== its for import_form ===========
    public function import_form() {
        $this->permission->check_label('database_import')->create()->redirect();
...
//            $config['upload_path'] = getcwd().'/assets/dbimport/';
...
//            }
//        }
//        $file_url = base_url() . "assets/dbimport/backup.sql";
        $file_url = base_url() . "assets/dbimport/wholesale.sql";
        
        $sql1 = "drop view if exists customer_transection_summary";
        $this->db->query($sql1);
        $sql2 = "drop view if exists sales_report";
        $this->db->query($sql2);
        $sql3 = "drop view if exists product_supplier";
        $this->db->query($sql3);
        $sql4 = "drop view if exists product_report";
        $this->db->query($sql4);
        $sql5 = "drop view if exists purchase_report";
        $this->db->query($sql5);
        $sql6 = "drop view if exists sales_actual";
        $this->db->query($sql6);
        $sql7 = "drop view if exists view_customer_transection";
        $this->db->query($sql7);
        $sql8 = "drop view if exists stock_history";
        $this->db->query($sql8);
        $sql9 = "drop view if exists view_person_transection";
        $this->db->query($sql9);
        $sql10 = "drop view if exists view_supplier_transection";
        $this->db->query($sql10);
        $sql11 = "drop view if exists view_transection";
        $this->db->query($sql11);
...
//         echo '<pre>';         print_r($tables);exit();
        ...
        $this->session->set_flashdata(array('message' => 'Successfully Imported '));
        redirect($_SERVER['HTTP_REFERER']);
    }
