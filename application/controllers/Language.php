...
class Language extends CI_Controller {
    ...
    public function phrase($carry = null) {
...
//    }
    ...
    public function editPhrase($language = null) {
        ...
        $config["total_rows"] = $this->phrases_count();
        $config["per_page"] = 50;
        ...
        $data['phrases'] = $this->phrases1($config["per_page"], $page);
//        echo '<pre>';        print_r($data['phrases']);die();

        $content = $this->parser->parse('language/phrase_edit', $data, true);
        $this->template->full_admin_html_view($content);
    }

    public function phrases_count() {
        if ($this->db->table_exists($this->table)) {
    ...
    public function phrases1($per_page, $page) {
        if ($this->db->table_exists($this->table)) {
